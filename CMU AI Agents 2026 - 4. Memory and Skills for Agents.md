...
![Agent Skills 标准的技能包结构：必需的 SKILL.md（YAML 元数据 + Markdown 正文）加可选的 scripts / references / assets 三个目录；页脚引用了 OpenHands 博客、Anthropic 的 Agent Skills 文章与 AgentSkills specification（视频约 13:54 处）](shots4/skill-structure.png)
...
![渐进披露三级机制：Level 1 的 YAML 元数据始终加载，Level 2 的 Markdown 正文在智能体判断相关时经工具调用加载，Level 3 的打包文件在读取时加载（视频约 17:34 处）](shots4/progressive-disclosure.png)
...
![SkillsBench 的核心结果：人工编写的技能把平均通过率从 33.9% 提升到 50.5%，但需要更少技能的任务受益更多，技能集更大时模型反而被分散注意力（视频约 26:21 处）](shots4/skillsbench-results.png)
...
![MemGPT 的记忆架构：有限上下文窗口被划分为系统指令（只读）、工作上下文（经函数读写）、FIFO 队列与输出缓冲区，外部有 Archival Storage 与 Recall Storage 两个存储，读写都由智能体自己发起的函数调用完成（视频约 32:25 处）](shots4/memgpt.png)
...
![Mem0 的冲突处理管线：抽取阶段由 LLM 从最近消息中提炼候选记忆，更新阶段检索 Top-K 相似记忆后由 LLM 在 ADD / UPDATE / DELETE / NOOP 四种操作间裁决（视频约 35:06 处）](shots4/mem0-update.png)
...
![AWM 的在线学习曲线：在地图工具基准上，基线智能体（蓝线）在约 10% 成功率附近徘徊，加入子任务记忆的 AWM（黑线）随任务数持续拉开差距，40 个样例后差距拉大到 22.5 个百分点，并能完成基线无法完成的复杂任务（视频约 51:14 处）](shots4/awm-curve.png)
...
![同一技能的两种表示：左侧文本 + 示例（任务描述加动作轨迹，优点 Flexible、缺点 Manual），右侧代码（search_product 函数，智能体作为工具直接调用）（视频约 55:25 处）](shots4/text-vs-code.png)
...
![失败对不同类型记忆的影响：Synapse（完整轨迹）与 AWM（工作流）加入负例后成功率反而下降，ReasoningBank（文本策略）加入负例后从 46.5 提升到 49.3，无记忆基线为 39.0（WebArena shopping，Gemini-2.5-flash）（视频约 1:06:22 处）](shots4/memory-types.png)
...
![judge 准确率消融：把真实标签按固定概率翻转来模拟不同准确率的裁判，judge 准确率 100% 时成功率 52.4，90% 时 49.7，80% 时 48.7，70% 时 49.7，60% 和 50%（抛硬币）时降至 47.6——性能在相当宽的 judge 质量区间内都有提升（视频约 1:08:50 处）](shots4/judge-ablation.png)
